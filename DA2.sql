...
go
create procedure DK_HC @Nhom nchar(10),@HC nchar(10),@SoLuong int
as
begin
	declare @dem int = (select count(*) from DangKyHC where @Nhom=DangKyHC.MaNhom and @HC=DangKyHC.MaHC);
	if @dem > 0
		return;
	else
		insert into DangKyHC values(@Nhom,@HC,@SoLuong);
end;

go
create procedure DK_DC @Nhom nchar(10),@DC nchar(10),@SoLuong int
as
begin
	declare @dem int = (select count(*) from DangKyDC where @Nhom=DangKyDC.MaNhom and @DC=DangKyDC.MaDC);
	if @dem > 0
		return;
	else
		insert into DangKyDC values(@Nhom,@DC,@SoLuong);
end;


/*Đăng ký phòng tn & Tập sd TB vs thiết bị*/
go
create procedure DK_TN @Phong nchar(10),@Nhom nchar(10),@Ngay date,@BD time,@KT time
as
begin
	declare @ti time = (select ThoiGianKT from DangKyTN where NgayTN=@Ngay and MaPTN=@Phong);
	if @ti > @BD
		return;
	else
		insert into DangKyTN values(@Phong,@Nhom,@Ngay,@BD,@KT);
end;

go
create procedure DK_TapSD @Phong nchar(10),@Nhom nchar(10),@Ngay date,@BD time,@KT time
as
begin
	declare @ti time = (select ThoiGianKT from DangKyTapSDTB where NgayTN=@Ngay and MaPTN=@Phong);
	if @ti > @BD
		return;
	else
		insert into DangKyTN values(@Phong,@Nhom,@Ngay,@BD,@KT);
